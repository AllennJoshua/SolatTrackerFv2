...
//const char* ssid     = "1qqqq";
//const char* password = "11111111";

//const char* ssid     = "HUAWEI-c2hG";
//const char* password = "allmight02";

const char* ssid     = "HUAWEI-2.4G-D9cV";
const char* password = "c46MvXMJ";


  int uploadeverywhen = 10;     // input interval to upload to db. unit is secs
  ...
  int voltagepin = 32;    // voltage sensor pins
  int voltageValue = 0;
  int battery = 0;
  ...
  unsigned long uniaddmins = 5;       // the amount of mins that will be added if coin is inserted
  unsigned long uniaddamount = 3;     // the amount of secs that will be added if coin is inserted
  unsigned long uploaddata = 0;       // initialize variable
...
void loop() {
  ...
  voltageValue = analogRead(voltagepin);    // reads the volage in voltage sensor
...
updatedtimerUp2 = timerUp2 - startTime;
  ...
  if (updatedtimerUp2 == 1){
              Serial.println("coin inserted 1");
    if (mins2 > 0){
...
//  Serial.println(sensorValue1);
    ...
    if (sensorValue2 == 1) {
                Serial.println("coin inserted 2");
          uploaddata = 1;
      if (updatedtimerUp2 >=1){
...
updatedtimerUp3 = timerUp3 - startTime;
  ...
  if (updatedtimerUp3 >= 1000000){
    ...
    if (sensorValue3 == 1) {
                Serial.println("coin inserted 3");
          uploaddata = 1;
      if (updatedtimerUp3 >=1){
...
//    Serial.println("   ");
  ...
  if (updatedtimerUp3 > 0){
    ...
    // code to upload every uploaddata seconds
    if (startTime >= uploaddata){
    uploaddata = startTime + uploadeverywhen;



 batstat = "11";
 batpercent = String(battery);
...
// port3stat = "bb";
  ...
  if (updatedtimerUp2 >= 1){
    ...
    port2stat = "Not_Charging";
  }
  
  if (updatedtimerUp3 >= 1){
    port3stat = "Charging";
  } else {
    port3stat = "Not_Charging";
  }
